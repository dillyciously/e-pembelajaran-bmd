...
class Tugas extends MY_Controller
{
    ...
    function reset_jawaban($tugas_id, $siswa_id)
    {
        ...
        if (is_pengajar() OR is_admin()) {
            ...
            $siswa = $this->siswa_model->retrieve($siswa_id);
            if (empty($siswa)) {
                show_error("Peserta tidak ditemukan.");
            }
            ...
            $this->session->set_flashdata('tugas', get_alert('success', 'Peserta berhasil dianggap belum mengerjakan.'));
            if ($tugas['type_id'] == 3) {
