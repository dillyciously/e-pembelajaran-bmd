...
class Login extends MY_Controller
{
    function index()
    {
        ...
        if ($this->form_validation->run('login') == TRUE) {
            ...
            if (empty($get_login)) {
                $this->session->set_flashdata('login', get_alert('warning', 'Username / Password salah.'));
                redirect('login');
            } else {
    ...
    function lupa_password()
    {
        ...
        if ($this->form_validation->run('lupa_password') == true) {
            ...
            $this->session->set_flashdata('lupa_password', get_alert('success', 'Segera konfirmasikan pengajuan reset password akun anda kepada Administrator (Whatsapp: 085214013670)'));
