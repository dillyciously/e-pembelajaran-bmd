...
class MY_Controller extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->update_link        = 'http://bpad.jakarta.go.id/portal/content/berita';
        $this->portal_update_link = 'http://bpad.jakarta.go.id/portal/content/berita';
        $this->bug_tracker_link   = 'http://aset.jakarta.go.id/';

        # load helper
        $this->load->helper(array('url', 'form', 'text', 'elearning', 'security', 'file', 'number', 'date', 'download', 'plugins'));

        $error_install = "Install" . anchor('setup', "&rarr; Halaman Install");
        try {
            check_db_connection();
        } catch (Exception $e) {
    ...
    function update_nis($nis = '')
    {
        if (!empty($nis)) {
            ...
            if ($result->num_rows() == 0) {
                ...
                $this->form_validation->set_message('update_nis', 'NIP sudah digunakan.');
                return false;
            }
        } else {
            $this->form_validation->set_message('update_nis', 'NIP di butuhkan.');
            return false;
        }
    }
    ...
    /**
     * Method untuk mendapatkan jawal Mata Pelajaran untuk siswa
     *
